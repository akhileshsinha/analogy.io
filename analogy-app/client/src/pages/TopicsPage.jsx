...
function useTopics(params) {
  ...
  const upvoteTopic = useMutation({
    mutationFn: (id) => http.post(`/topics/${id}/upvote`),
    onSuccess: () => {
      // refresh lists and any open topic
  ...
  return (
    <div>
      ...
      >
        ...
        <input
          name="purposes"
          placeholder="purposes (csv)"
          defaultValue={sp.get("purposes") || ""}
          onChange={onChange}
        />
        <select
          name="sort"
          defaultValue={sp.get("sort") || "popularityScore"}
          onChange={onChange}
        >
      ...
      {data?.items?.map((t) => (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#555" }}>
            ...
            <span>Responses: {t.responsesCount}</span> ·{" "}
            <span>Pop: {t.popularityScore}</span>
            <button
              style={{ marginLeft: 8 }}
              onClick={() => upvoteTopic.mutate(t._id)}
            >
              ▲ Upvote
            </button>
          </div>
      ...
      {data && (
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <button
            disabled={Number(sp.get("page") || 1) <= 1}
            onClick={() =>
              setSp((p) => {
          ...
          <span>
            Page {data.page} / {Math.ceil(data.total / data.limit) || 1}
          </span>
          <button
